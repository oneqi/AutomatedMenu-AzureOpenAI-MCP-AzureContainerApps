...
try {
    ...
    Write-Host "Resource Group '$($rg.ResourceGroupName)' created successfully in location '$($rg.Location)'." -ForegroundColor Green
} catch {
    Write-Host "Failed to create Resource Group '$global:resourceGroup'. Error: $_" -ForegroundColor Red
    exit 1
}

# Exporting Resource Group as global variable
$global:resourceGroup = $rg.ResourceGroupName
Write-Host "`nExported Resource Group name as global variable" -ForegroundColor Cyan
